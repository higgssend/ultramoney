...
const Overdue: React.FC = () => {
  ...
  const handleWhatsAppNotice = (loan: Loan) => {
    ...
    const message = `Hola *${loan.clientName}*, le saludamos de *${companySettings.name || 'UltraMoney'}*. Le recordamos que su cuota del préstamo #${formatLoanId(loan.id)} por valor de *RD$ ${loan.remainingBalance.toLocaleString()}* presenta atrasos. Por favor contactarnos a la brevedad para coordinar su regularización. ¡Gracias!`;
  ...
  const handleWhatsAppGuarantorNotice = (loan: Loan, guarantorName: string, guarantorPhone: string) => {
    ...
    const message = `Estimado/a *${guarantorName}*,\n\nLe saludamos de *${companySettings.name || 'UltraMoney'}* en relación al crédito solidario de *${loan.clientName}* (Préstamo #${formatLoanId(loan.id)}).\n\n⚠️ Le notificamos que dicho préstamo presenta atrasos en sus pagos con un balance vencido de *RD$ ${loan.remainingBalance.toLocaleString()}*.\n\nEn virtud de su condición de *GARANTE SOLIDARIO Y MANCOMUNADO*, le solicitamos comunicarse con nosotros para coordinar la regularización de la cuenta.\n\nGracias por su pronta atención.`;
    window.open(`https://wa.me/${cleanPhone}?text=${encodeURIComponent(message)}`, '_blank');
  };
              ...
              return (
                <div key={loan.id} className="bg-white dark:bg-slate-900 rounded-3xl shadow-sm border border-slate-100 dark:border-slate-800 p-6 relative overflow-hidden group hover:shadow-xl hover:shadow-rose-100/50 dark:hover:shadow-none transition-all hover:-translate-y-1 space-y-4">
                    ...
                    <div className="flex justify-between items-start relative z-10">
                        <div>
                            <h3 className="font-bold text-lg text-slate-800 dark:text-white">{loan.clientName}</h3>
