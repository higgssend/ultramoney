                 ...
                 <div className="bg-slate-50/70 dark:bg-slate-800/40 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-700/60 space-y-4">
                     <div className="flex items-center gap-3">
                         ...
                         <div>
                             ...
                             <p className="text-xs text-slate-500">Contratos iniciales completos, pagarés notariales, estados de cuenta y cartas de saldo cero</p>
                         </div>
                     </div>
                         ...
                         <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                             {legalDocs.map(doc => (
                                 ...
                                 >
                                     <div>
                                         <div className="flex justify-between items-start mb-3">
                                             <div className="w-10 h-10 bg-indigo-50 dark:bg-indigo-900/30 text-indigo-600 dark:text-indigo-400 rounded-xl flex items-center justify-center font-bold border border-indigo-100 dark:border-indigo-800">
                                                 <FileText className="w-5 h-5" />
                                             </div>
                                             <span className="text-[10px] px-2.5 py-1 rounded-full font-extrabold uppercase bg-indigo-100 text-indigo-700 dark:bg-indigo-900/50 dark:text-indigo-300">
                                                 Legal
                                             </span>
                                         </div>

                                         <h4 className="font-bold text-slate-800 dark:text-white text-xs line-clamp-2 mb-1">{doc.title}</h4>
                                         <p className="text-[10px] text-slate-400">Generado: {doc.uploadDate}</p>
                                     </div>

                                     <div className="flex items-center gap-1.5 pt-3 mt-3 border-t border-slate-100 dark:border-slate-700/60">
                                         <button
                                             onClick={() => window.open(doc.fileUrl, '_blank')}
                                             className="flex-1 py-1.5 bg-indigo-50 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 hover:bg-indigo-100 rounded-xl text-xs font-bold flex items-center justify-center gap-1 transition-all"
                                         >
                                             <Eye className="w-3.5 h-3.5" /> Abrir Documento
                                         </button>
                                         <button
                                             onClick={() => {
                                                 navigator.clipboard.writeText(doc.fileUrl);
