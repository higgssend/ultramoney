...
export const BankReconciliation: React.FC = () => {
  ...
  const handleOpenMatchModal = (deposit: BankDeposit) => {
    ...
    if (matchedClient) {
      ...
      const activeLoan = loans.find(l => l.clientId === matchedClient?.id && (l.status === LoanStatus.ACTIVE || l.status === LoanStatus.DEFAULTED));
      if (activeLoan) {
  ...
  const smartSuggestions = useMemo(() => {
    ...
    // Exact amount match
    loans.forEach(loan => {
      if (loan.status !== LoanStatus.ACTIVE && loan.status !== LoanStatus.DEFAULTED) return;
  ...
  const clientActiveLoans = useMemo(() => {
    ...
    return loans.filter(l => l.clientId === matchSelectedClientId && (l.status === LoanStatus.ACTIVE || l.status === LoanStatus.DEFAULTED));
                      ...
                      modalFilteredClients.map(c => (
                        <div
                          ...
                          onClick={() => {
                            ...
                            const firstLoan = loans.find(l => l.clientId === c.id && (l.status === LoanStatus.ACTIVE || l.status === LoanStatus.DEFAULTED));
