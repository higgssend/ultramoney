...
# Insert just before the submit button in the modal
if "Portal Web de Cliente" not in content.split("isEditModalOpen")[1]:
    # We will just replace `<div className="flex justify-end gap-3 pt-6 border-t dark:border-slate-700">`
    # inside the edit modal
    
    # We can use regex to replace the first occurrence of the button container after isEditModalOpen
    parts = content.split("isEditModalOpen")
    if len(parts) > 1:
        ...
        content = "isEditModalOpen".join(parts)
        
    with open(file_path, "w", encoding="utf-8") as f:
        f.write(content)
    print("ClientDetail edit modal patched!")
else:
